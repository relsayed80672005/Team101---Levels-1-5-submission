...
2. `CORE-02` Letter grades use these exact boundaries with no upward rounding: `A` for `90.00-100.00`, `B` for `80.00-89.99`, `C` for `70.00-79.99`, `D` for `60.00-69.99`, and `F` below `60.00`.
...
5. `CORE-05` The lowest quiz score is dropped only when a student has at least three quiz assignments in the course.
...
34. `STATE-02` When duplicate grade rows are present for the same student and assignment, the later row in the file is treated as the corrected grade.
35. `STATE-03` Withdrawn students remain visible to validation and audit workflows but are excluded from ranking and aggregate reporting.
36. `STATE-04` The `validate` command exits with a non-zero status code when validation issues are present and `0` when no validation issues are present.
37. `STATE-05` The `audit` command includes invalid grade records, duplicate grade rows, and policy violations in its output.
38. `STATE-06` Supported student status values are `active`, `inactive`, and `withdrawn`. Unknown status values default to `active`.
39. `STATE-07` An excused grade row may omit the numeric score field.
40. `STATE-08` The loader reads students, assignments, and grades as separate inputs and reports missing-file problems as validation issues.
